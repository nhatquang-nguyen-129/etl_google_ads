...
# ETL for Google Ads campaign insights
    ...
    # Load
        ...
        if dags_start_date <= dags_end_date:
            print(
                "🔄 [DAGS] Waiting "
                f"{DAGS_INSIGHTS_COOLDOWN} second(s) cooldown before processing next date..."
            )
...
# ETL for Google Ads campaign metadata
    ...
    for attempt in range(1, DAGS_CAMPAIGN_ATTEMPTS + 1):
        ...
        if not failed_campaign_ids:
            print(
                "✅ [DAGS] Successfully triggered to extract Google Ads campaign metadata with "
        ...
        if not retryable:
            print(
                "❌ [DAGS] Failed to extract Google Ads campaign metadata for "
...
# Materialization with dbt
    print("🔄 [DAGS] Trigger to materialize Google Ads campaign insights with dbt...")
